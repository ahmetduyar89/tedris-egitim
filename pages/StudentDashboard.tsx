...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, onLogout, onNavigateToContent }) => {
  ...
  const loadStudentData = useCallback(async () => {
    ...
    try {
        ...
        // Fetch assignments and submissions in parallel (optimize N+1 query problem)
        const [assignmentsSnapshot, submissionsSnapshot] = await Promise.all([
            db.collection('assignments').where('studentId', '==', user.id).get(),
        ...
        const assignmentsList = assignmentsSnapshot.docs.map((doc: any) => {
