...
const StudentDetailPage: React.FC<StudentDetailPageProps> = ({ user, student, onBack, onLogout, onStudentUpdate }) => {
    ...
    const renderOverviewTab = () => (
        <StudentOverviewTab
            ...
            onAddPDFTest={() => setIsCreatingPDFTest(true)}
        />
    );
            ...
            {viewingReport ? (
                ...
                <main className="p-4 md:p-8">
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 md:p-6 rounded-xl shadow-md mb-6 md:mb-8 text-white relative overflow-hidden">
                            {/* Decorative Pattern */}
                            ...
                            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 relative z-10">
                                <div>
                                    <h2 className="text-2xl md:text-3xl font-bold font-poppins text-white leading-tight">{student.name}</h2>
                                    <p className="text-blue-100 font-medium">{student.grade === 4 ? 'İlkokul' : `${student.grade}. Sınıf`}</p>
                                </div>
                                <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full lg:w-auto mt-2 lg:mt-0">
                                    ...
                                    <button onClick={() => setIsCreatingFlashcard(true)} className="flex-1 sm:flex-none bg-white/10 text-white px-3 md:px-4 py-2 rounded-xl hover:bg-white/20 flex items-center justify-center space-x-2 text-sm md:text-base border border-white/10 backdrop-blur-sm transition-all">
                                        <span className="text-lg">🔄</span>
                                        <span className="whitespace-nowrap">Flashcard</span>
                                    </button>
                                    <button onClick={() => setIsCreatingPDFTest(true)} className="flex-1 sm:flex-none bg-white/10 text-white px-3 md:px-4 py-2 rounded-xl hover:bg-white/20 flex items-center justify-center space-x-2 text-sm md:text-base border border-white/10 backdrop-blur-sm transition-all">
                                    ...
                                    <button onClick={() => setIsSelectingDiagnosisTest(true)} className="flex-1 sm:flex-none bg-orange-500 text-white px-3 md:px-4 py-2 rounded-xl hover:bg-orange-600 flex items-center justify-center space-x-2 text-sm md:text-base font-semibold shadow-md transition-all">
                                        <span className="text-base md:text-lg">🩺</span>
                                        <span className="whitespace-nowrap">Tanı Testi</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100 mb-6 md:mb-8">
                            <nav className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2" aria-label="Tabs">
                                <button
                                    ...
                                    className={`flex-1 flex items-center justify-center space-x-2 px-3 py-3 rounded-xl transition-all duration-200 ${activeTab === 'overview'
